...
# 1.1 Required Admin Files for Governed Sub‑Units (Mandatory)

Every governed sub‑unit inside an admin‑grade repository MUST contain the full admin file set:
...
- each standard under /standards/<standard>/  
- each specification under /specifications/<standard>/  
- each architecture family under /architecture/<family>/  

Each governed sub‑unit is independently:
...
Therefore each MUST contain its own complete admin surface.

This rule is mandatory, non‑optional, and normative.
...
Each folder under ARCHITECTURE/ MUST contain the full governed sub‑unit admin file set.
...
Each folder under STANDARDS/ MUST contain the full governed sub‑unit admin file set.
...
Each folder under SPECIFICATIONS/ MUST contain the full governed sub‑unit admin file set.
